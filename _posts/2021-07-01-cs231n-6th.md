---
layout: post
title: '[CS231n]Lecture 6-Training Neural Networks, Part 1'
...
입력값이 커지거나 작아지면 기울기가 0에 가까워지는 현상이 발생한다. 이것이 중첩되고 곱해지면서 gradient가 0에 수렴해 버리고, 학습 효율이 떨어진다. 뿐만 아니라 미분값의 최댓값이 1/4밖에 되지 않는다.

2. 출력값이 zero-centered가 아니며 항상 양수의 값을 가진다. 따라서 다음 layer가 항상 양의 입력값만을 받게 되어 역전파 과정에서 무조건 양 혹은 무조건 음의 방향으로만 진행하게 된다. 이는 수렴 속도가 느려지는 결과를 야기한다.
...
3. 지수함수는 computational cost가 비싸다.

이러한 이유로 최근에는 sigmoid 함수를 거의 사용하지 않는다.
...
음수 영역만 조금 다르게 한 것으로, ReLU의 모든 장점을 다 가지고 있으면서 gradient가 절대 죽지 않는다. Leaky는 새고 있다는 뜻이다. 비슷한 것으로, Parametric Rectifier(PReLU)가 있다.
