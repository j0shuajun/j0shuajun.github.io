...
마지막으로 VM 인스턴스로 돌아가서 외부 IP를 확인한 후, 다음의 명령어를 터미널에 입력하면 VM 인스턴스에 접속할 수 있다. 처음 접속하는 경우, `nvidia-driver`를 설치하라는 메시지가 나올텐데 전부 `yes`를 입력하면 된다.
...
예2) sudo gsutil cp gs://sample_bucket/data ~/projects
```