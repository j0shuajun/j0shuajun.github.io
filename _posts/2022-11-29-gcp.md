...
새로운 인스턴스 만들기 클릭 후, 이름을 설정한 다음 목적에 맞는 머신을 구성한다. 필자는 딥러닝을 위한 **GPU**가 필요했기 때문에 GPU 카테고리에 머신을 구성했다. 리전과 영역에 따라 선택할 수 있는 GPU가 다르기 때문에 원하는 GPU가 선택한 리전에 없다면, 리전을 변경하자. (아무 리전을 선택해도 잘 되는 것인지 모르겠다)
...
VM 인스턴스로 돌아가서 외부 IP를 확인한 후, 터미널에 다음의 명령어를 입력한다. 처음 접속하는 경우, 이것저것 설치하라는 메시지가 나올텐데 전부 `yes` 또는 `y` 입력하면 된다.
